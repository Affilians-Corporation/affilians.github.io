...
export class Player extends Component {
    ...
    public is_this_player: boolean = false;
    @property({type: Node})
    ...
    onCollision(selfCollider: Collider2D, otherCollider: Collider2D, contact: IPhysics2DContact | null) {
        switch (otherCollider.tag) {
            ...
            case 1:
                ...
                break
            case 2:
                find("GameManager").getComponent(network_manager).room.send("grab_ball")
            ...
            case 3:
                if (this.holding == "Ball") {
                    ...
                    find("GameManager").getComponent(network_manager).room.send("goal_scored", {"team": "a"})
                }
            ...
            case 5:
                ...
                find("GameManager").getComponent(network_manager).room.send("died");
                var that = this;
                var spawn_point_x = gm.getComponent(network_manager).room.state["players"].get(this.node.name)["spawn_point_x"]
                var spawn_point_y = gm.getComponent(network_manager).room.state["players"].get(this.node.name)["spawn_point_y"]
                setTimeout(function(){
                    that.node.setPosition(new Vec3(spawn_point_x, spawn_point_y, 0));
    ...
    start() {
        ...
        gm = find("GameManager")
        if(this.node.name === find("GameManager").getComponent(network_manager).room.sessionId) {
            this.is_this_player = true;
        }
    }
    ...
    update(deltaTime: number) {
        ...
        if ((current_position.x != old_position.x || current_position.y != old_position.y) && this.is_this_player) {
